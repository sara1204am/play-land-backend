...
let LoginController = class LoginController {
    ...
    async login(loginDto, request) {
        ...
        if (!user) {
            throw new common_1.UnauthorizedException('There is not a valid user');
        }
        const tokenData = await this.authService.login(user, loginDto.ttl);
        if (tokenData) {
            await this.loginService.saveLoginData(user.id, tokenData.id, request?.headers?.['user-agent'], 'web');
        }
        return tokenData;
    }
};
